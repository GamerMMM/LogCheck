...
class VirtualTextWidget(QWidget):
    ...
    def load_file(self, file_path: str, line_offsets: List[int]) -> bool:
        """
        加载文件进行显示
...
class BigFileViewer(QMainWindow):
    ...
    def setup_drag_drop(self):
        ...
        # 创建拖拽提示标签
        self.drag_label = QLabel("📁 拖拽文件到此处打开", self.drag_overlay)
        ...
        overlay_layout.addWidget(self.drag_label)
    ...
    def _is_valid_file(self, file_path: str) -> bool:
        ...
        supported_extensions = {
            '.txt', '.log', '.text',                    # 文本文件
            '.py', '.js', '.java', '.cpp', '.c', '.h',  # 代码文件
            '.cs', '.php', '.rb', '.go', '.rs', '.kt',  # 更多代码文件
            '.csv', '.json', '.xml', '.yaml', '.yml',   # 数据文件
            '.conf', '.cfg', '.ini', '.config',         # 配置文件
            '.md', '.rst', '.tex',                      # 文档文件
            '.sql', '.sh', '.bat', '.ps1',              # 脚本文件
            ''  # 无扩展名文件（如Linux配置文件）
        }
    ...
    def _is_text_file(self, file_path: str) -> bool:
        ...
        try:
            ...
            return text_ratio > 0.7  # 如果70%以上是可打印字符，认为是文本文件
    ...
    def _show_drag_overlay(self):
        ...
        if not self.drag_active:
            ...
            # 调整覆盖层大小以覆盖整个中央widget
            if self.centralWidget():
                central_rect = self.centralWidget().geometry()
                self.drag_overlay.setGeometry(central_rect)
                self.drag_overlay.show()
                self.drag_overlay.raise_()  # 确保在最顶层
    ...
    def _load_dropped_file(self, file_path: str):
        """
        加载拖拽的文件
        ...
        # 更新状态显示
        self.status_label.setText(f"🔄 正在通过拖拽加载文件... 大小: {size_mb:.1f}MB")
    ...
    def resizeEvent(self, event):
        ...
        # 如果拖拽覆盖层正在显示，调整其大小
        if self.drag_active and self.centralWidget():
            central_rect = self.centralWidget().geometry()
            self.drag_overlay.setGeometry(central_rect)
    ...
    def init_ui(self):
        ...
        # 🆕 拖拽提示标签
        self.drag_hint_label = QLabel("💡 提示：可直接拖拽文件到窗口打开")
        ...
        toolbar_layout.addWidget(self.drag_hint_label)
        ...
        # 状态信息
        self.status_label = QLabel("🚀 就绪 - 支持GB级大文件高速搜索 | 📂 可拖拽文件直接打开")
    ...
    def close_file(self):
        ...
        self.status_label.setText("📁 文件已关闭 | 📂 可拖拽文件直接打开")
        self.clear_search_results()
    ...
    def on_search_progress(self, progress, found_count):
        """搜索进度更新"""
